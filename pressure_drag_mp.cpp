...
int main(){
        int n=6;//degree of Bezier Curve
        Control Pi[n+1];
...
//
        ...
        int max_index = 0;
        int max_x_coord=point[0].x;
        for(int i=0;i< num_points;i++)
        {
            if(point[i].x>max_x_coord)
               {
                max_x_coord=point[i].x;
                max_index=i;
               }
        }

        //cout<<"max index"<<max_index<<"max x coordinate"  <<max_x_coord<<endl;
        ...
        {            
            if(i <= max_index){
                ...
                point[i].A = A_throat + (Pi[6].y - point[i].y);
                point[i].ratio = point[i].A / A_throat;
            }
